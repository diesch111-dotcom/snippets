...
# contents of a potential csv (here a semicolon separated value) file
...
# write a csv data file (exercise)
states_file = "/home/admin123/AAtest_py/aatest_gz/US_states.csv"
with open(states_file, "w") as fout:
    fout.write(data_str)

# read the csv data file back in (exercise)
with open(states_file, "r") as fin:
    states_data_str = fin.read()

# create a list of [state, capital, date, flower, bird] lists
# sort by state name just to make sure
data_list = sorted(line.split(';') for line in states_data_str.split('\n') 
                   if line)
...
bird = "Robin"
print("US States with '{}' as state bird:".format(bird))
...
print("US States with '{}' as state flower:".format(flower))
for state in instance_list:
...
first = 'M'
print("All US states that start with '{}':".format(first))
for state in instance_list:
...
US States with 'Robin' as state bird:
Connecticut
Michigan
Wisconsin
========================================
US States with 'Apple Blossom' as state flower:
Arkansas
Michigan
========================================
All US states that start with 'M':
Maine
Maryland
Massachusetts
Michigan
