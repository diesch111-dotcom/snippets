...
this color list represents a list of colors where each line/item is:
name of color, hex string of color, color tuple

name is similar to XConsortium names
hexstr is in format '#rrggbb' or '#RRGGBB' hex values 0 to f
    so '#ff0000' or '#FF0000' would be full red
tuple is (r, g, b)
    r, g, b are integer values red, green, blue from 0 to 255
    so (255, 0, 0) would be full red
    and (0, 0, 255) would be full blue

the tkinter GUI uses 'names' or 'hex strings' for color
other python GUI modules like PIL, Pygame, vpython differ

use list comprehension or dictionary comprehension to convert
the color_list to your needs: 

eg.    
#for instance a dictionary with 'name:hexstr' pairs
color_dict = { name: hexstr for name, hexstr, tup in color_list}

eg.
#list all the colors names starting with char 'g'
...
# color names sorted in alphabetical order
color_list = \
