#!/usr/bin/env python3
# -*- coding: utf-8 -*-
''' SortingAlgorithms8.py

Timing 8 different Python sorting algorithms
each function is given the same list of integers (fresh copy each time)

note:
time.clock() removed starting with Python38
replaced with more precise time.perf_counter() instead
...
@print_timing
def shell_sort(list2):
    inc = len(list2) // 2
    while inc:
        for i, el in enumerate(list2):
            while i >= inc and list2[i - inc] > el:
                list2[i] = list2[i - inc]
                i -= inc
            list2[i] = el
...
def bubble_sort(list2):
    ...
    for i in range(0, len(list2) - 1):
        # as suggested by kubrick, just a bit faster
        swap_test = False
...
if __name__ == "__main__" :
    print("timing 8 sorting algorithms with a list of 10000 integers:")
...
''' possible result...
timing 8 sorting algorithms with a list of 10000 integers:
adaptive_merge_sort took 1.045ms
shell_sort took 15.454ms
bubble_sort took 3664.254ms
heap_sort took 17.278ms
insertion_sort took 1867.112ms
merge_sort took 17.233ms
quick_sort took 10.083ms
selection_sort took 1525.835ms
'''
